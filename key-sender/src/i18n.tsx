...
const translations: Record<Language, Record<string, string>> = {
  en: {
    ...
    placeholderGeminiModel: "gemini-3.1-flash-live-preview",
  ...
  ko: {
    ...
    explainDesc2: "이 페이지에서는 입력하신 Gemini API Key, Naver Client ID 및 Secret을 규격화된 JSON 객체 구조로 변환한 다음, 사람이 들을 수 있는 청각 음파 대역(Audible Fast) 또는 초음파 대역(Ultrasound)으로 부호화(FSK 변조)하여 소리로 송출(TX)합니다. 수신 장치는 마이크를 통해 이 소리를 듣고 다시 데이터로 복원하게 됩니다.",
  ...
  ja: {
    ...
    explainDesc2: "このページでは、入力された Gemini API Key, Naver Client ID および Secret を規格化された JSON オブジェクト構造に変換し、人が聞き取れる可聴音波帯域（Audible Fast）または超音波帯域（Ultrasound）に符号化（FSK変調）して音として送出（TX）します。受信デバイスはマイクを介してこの音を聞き取り、再びデータに復元します。",
